...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
   ...
   # scores-=np.reshape(np.max(scores,axis=1),(N,-1))
    ...
    true_class=np.zeros_like(scores)
    true_class[np.arange(N),y]=1
    loss=-np.sum(true_class*np.log(scores))/N+0.5*reg*np.sum(W1*W1)+0.5*reg*np.sum(W2*W2)
    ...
    dh=np.zeros_like(hidden)
    p=scores-true_class
    dW2+=np.dot(hidden.T,p)/N+reg*W2
    dh=p.dot(W2.T)
    dh/=N
    dh[hidden <= 1e-5] = 0
